...
class WPLMS_Register{
    ...
    function wplms_enqueue_head() {
        ...
        $gutenberg = apply_filters('wplms_enqueue_head_gutenberg',0);
        if( ! is_admin() || $gutenberg ){
         ...
         if(!empty($google_fonts) && is_array($google_fonts)){
            ...
            $load_wplms_styles_scripts = 1;//apply_filters('wplms_load_styles_scripts',true);
            if($load_wplms_styles_scripts){
          if (function_exists('is_bbpress') && is_bbpress()){
            wp_enqueue_style( 'bbpress-css', VIBE_URL .'/assets/css/bbpress.min.css',array('bbp-default'),WPLMS_VERSION);
          }          
         ...
         if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) || (function_exists('vibe_check_plugin_installed') && vibe_check_plugin_installed( 'woocommerce/woocommerce.php')) || function_exists('WC')) {     
          ...
          if(function_exists('is_wplms_4_0') && is_wplms_4_0() && vibe_get_option('offload_scripts')){
            ...
            wp_enqueue_style('wplms-core', VIBE_URL.'/assets/css/core.min.css',array(),WPLMS_VERSION);
            wp_enqueue_style('wplms-v4style', VIBE_URL.'/assets/css/v4.min.css',array(),WPLMS_VERSION);  
            if(empty(vibe_get_option('header_scripts'))){
              wp_enqueue_style('wplms-header', VIBE_URL.'/assets/css/header.min.css',array(),WPLMS_VERSION);  
            }
      
            if(!vibe_get_option('offload_fonts')){
              wp_deregister_style( 'font-awesome');
              wp_enqueue_style('font-awesome', VIBE_URL.'/assets/fonts/font-awesome.min.css',array(),WPLMS_VERSION);
              wp_enqueue_style('font-icons', VIBE_URL.'/assets/css/fonticons.css',array(),WPLMS_VERSION);
            }
          ...
          wp_enqueue_style('vicons', VIBE_URL.'/assets/vicons.css',array(),WPLMS_VERSION);
        ...
        if(!is_admin()){ 
          ...
          if(function_exists('is_wplms_4_0') && is_wplms_4_0() && vibe_get_option('offload_scripts')){
              if(vibe_get_option('offload_scripts') == 2){
                ...
                wp_localize_script( 'wplms-js', 'v4', $params );
              }else{
                wp_enqueue_script( 'dependent-js', VIBE_URL .'/assets/js/wplms.jquery.min.js',array('jquery'),WPLMS_VERSION);
                wp_enqueue_script( 'wplms-js', VIBE_URL .'/assets/js/v4.js',array('dependent-js'),WPLMS_VERSION);    
                wp_localize_script( 'wplms-js', 'v4', $params );
              }
    ...
    function register_vibe_menus() {
      ...
      if(function_exists('register_sidebar')){     
        ...
        register_sidebar( array(
            ...
            'id' => 'topfootersidebar',
            'before_widget' => '<div id="%1$s" class="'.$topspan.'"><div class="footerwidget %2$s">',
        ...
        register_sidebar( array(
          ...
          'before_widget' => '<div id="%1$s" class="'.$bottomspan.'"><div class="footerwidget %2$s">',
          'after_widget' => '</div></div>',
          'before_title' => '<h4 class="footertitle"><span>',
          'after_title' => '</span></h4>',
        ...
        $google_webfonts=get_option('google_webfonts');
            if(!isset($google_webfonts) || $google_webfonts ==''){
                ...
                if(isset($api_key) && $api_key){
                  ...
                  $fonts = $wp_filesystem->get_contents(plugin_dir_path(__FILE__).'fonts.json');
                  add_option( 'google_webfonts', "$fonts",'', 'no');
                }
            }
    } 
