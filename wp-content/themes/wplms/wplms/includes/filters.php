...
class WPLMS_Filters{
    ...
    private function __construct(){
		...
		/* ===== INSTRUCTOR PRIVACY ====== */
		/* v4 */
		add_filter('wplms_frontend_cpt_query',array($this,'wplms_instructor_privacy_filter'),10,2);
		/* End v4 */
    	...
    	//ajax login form showing bbpress form
    	add_filter('bbp_login_widget_title',array($this,'remove_ajax_reg_login_from_wp_login'));

    	add_filter('evo_header_template',array($this,'load_wplms_header_bro'));

    	//vibeBP
    	add_filter('wplms_login_trigger',array($this,'vibebp_single_page'));
    }
	
	function vibebp_single_page($trigger){

		if(function_exists('vibebp_get_setting') && vibebp_get_setting('bp_single_page') && empty(vibebp_get_setting('global_login'))){
			$trigger = '<a href="'.get_permalink(vibebp_get_setting('bp_single_page')).'" rel="nofollow" class=" vibebp-login"><span>'.__('LOGIN','vibe').'</span></a>';
		}
		return $trigger;	
	}

    function load_wplms_header_bro($style){
    	$header = vibe_get_header();
    	if(!empty($header)){
    		return $header;
    	}
    	return $style;
    ...
    function remove_ajax_reg_login_from_wp_login($x){
	...
	function nav_search($args){
		...
		if(vibe_get_customizer('header_style') !== 'mooc'){
