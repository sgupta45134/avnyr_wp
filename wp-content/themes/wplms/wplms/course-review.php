...
if($post->comment_status == 'open'){
  ...
  if(is_user_logged_in() && !empty($_GET['replytocom'])){
     ...
     $comment_field='<p>' . '<textarea id="comment" name="comment" class="form_field" rows="8" ">'.$content.'</textarea></p>';
      comment_form(array('fields'=>$fields,'comment_field'=>$comment_field,'label_submit' => __('Post Reply','vibe'),'title_reply'=> '<span>'.__('Post Review','vibe').'</span>','logged_in_as'=>'','comment_notes_after'=>'' ));
  }
