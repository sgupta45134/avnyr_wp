...
class WPLMS_Actions{
    ...
    private function __construct(){
		...
		add_action('wp_head_wplms_login',array($this,'show_loggedin'));
		add_action('wp_print_scripts', array($this,'deregister_or_dequeue_scripts'), 20);
		...
		if(class_exists('WPLMS_tips') && method_exists('WPLMS_tips', 'init')){
			...
			if(!empty($tips->settings) && !empty($tips->settings['woocommerce_account'])){
				...
				if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) )  || (function_exists('vibe_check_plugin_installed') && vibe_check_plugin_installed( 'woocommerce/woocommerce.php')) || class_exists('WooCommerce')) {
				...
				if ( in_array( 'paid-memberships-pro/paid-memberships-pro.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) )  || (function_exists('vibe_check_plugin_installed') && vibe_check_plugin_installed( 'paid-memberships-pro/paid-memberships-pro.php'))) {
		...
		add_action('template_redirect',array($this,'wplms_check_single_activity_view'),1);
		...
		add_action('wp_ajax_import_wplms_demos_elementor_template', array($this,'import_wplms_demos_elementor_template'));
		...
		//to be removed below
		add_action('wp_head',array($this,'add_dark_background'),99);
    }
    ...
    function add_dark_background(){
    	$bg_color = '';$theme_skin = '';
    	if(function_exists('vibe_get_customizer')){
    		$bg_color = vibe_get_customizer('body_bg');
    		$theme_skin = vibe_get_customizer('theme_skin');
    	}
    	if(!empty($theme_skin) && $theme_skin == 'dark' && !empty($bg_color)){
	    	?>
	    	<style type="text/css">
    ...
    function nav_search(){
    	...
    	if($course_search ==2 || $course_search ==3){
    		...
    		echo '<a id="new_searchicon"><i class="vicon vicon-search"></i></a>';
    	}
    ...
    function switch_demo_layout(){
	    ...
	    $dir = get_template_directory().'/setup/installer/content/';
	    $demo = $_POST['demo'];
		if ( is_dir( $dir ) ) {
    ...
    function import_wplms_ele_temps($path){

	  if(class_exists('Elementor\Plugin')){
	    $source = Elementor\Plugin::$instance->templates_manager->get_source( 'local' );
	    //$path = '/Applications/MAMP/htdocs/wplms/elementordemos/demo3/demo3.json';
	    $imported_items = $source->import_template( basename( $path ),$path);
	...
	function import_wplms_demos_elementor_template(){
		if ( !isset($_POST['security']) || !wp_verify_nonce($_POST['security'],'switch_demo_layouts') || empty($_POST['demo'])){
	       _e('Security check Failed. Contact Administrator.','vibe');
	       die();
	    }
	    //find home page layout in json file and import it 
	    $dir = get_template_directory().'/setup/elementor_templates/';
	    $style = $_POST['demo'];
		if ( is_dir( $dir ) ) {
			$json_url = $dir.$style.'.json';
			$this->import_wplms_ele_temps($json_url);
		}
		die();
	}
    ...
    function switch_demo_homes(){
	    ...
	    $style = $_POST['demo'];
	    $url = 'https://wplms.s3.ap-south-1.amazonaws.com/demodata/'.$style;
		//if ( is_dir( $dir ) ) {
			$json_url = $url.'/default.json';
			...
			if(!empty($json)){
				...
				foreach ($json as $key => $page) {
					...
					if(!empty($page) && !empty($page['post_name']) && ($page['post_name'] == 'home' || (($style=='demo_4' || $style=='demo_4_academy') && $page['post_name'] == 'home-alt'))){
						$metas = $page['meta'];
						unset($page['meta']);
						unset($page['post_id']);
						unset($page['guid']);
						...
						if ( !(is_wp_error( $homepage )) && !empty($homepage) ) {
					        ...
					        if(in_array($style,array('demo17'))){
								$slider_array = array($url."/demo17-1.zip",$url."/demo17-2.zip",$url."/demo17-3.zip");
							}
							...
							foreach($metas as $key => $meta){
								...
								if(!empty($meta)){
									...
									if($key == '_elementor_data'){
										update_post_meta($homepage, $key,addslashes($meta));
									}else{
										update_post_meta($homepage, $key,$meta);
									}
									//update_post_meta($homepage, $key,$meta);
							...
							if ( !empty($metas['_elementor_data']) || !!empty($metas['_elementor_css']) ) {
                                if(class_exists('Envato_Theme_Setup_Wizard')){
                                    
                                    $setup = Envato_Theme_Setup_Wizard::get_instance();
                                    $setup->elementor_post( $homepage );
                                }
                            }
    ...
    function add_loading_css(){
    	...
    	$page_loader = vibe_get_option('page_loader');
   		if(!empty($page_loader) && !is_customize_preview()){
	    	...
	    	?>
	    	<style>
	    	body.loading .pusher:before{content:'';position:fixed;left:0;top:0;width:100%;height:100%;background:rgba(255,255,255,.95);z-index:999}body.loading.pageloader2 .global:after,body.loading.pageloader2 .global:before{content:'';position:fixed;left:50%;top:50%;margin:-8px 0 0 -8px;width:15px;height:15px;border-radius:50%;z-index:9999;background:#009dd8;z-index:9999;animation:flipzminus linear 1s infinite}body.loading.pageloader2 .global:after{animation:flipzplus linear 1s infinite}@keyframes flipzminus{0%{transform:translateX(-30px);opacity:1}50%{transform:translateX(0);opacity:.5}100%{transform:translate(30px);opacity:1}}@keyframes flipzplus{0%{transform:translate(30px);opacity:1}50%{transform:translateX(0);opacity:.5}100%{transform:translateX(-30px);opacity:1}}
	    	</style><script>window.addEventListener( 'load', function( event ) {if(document.querySelector('body') && document.querySelector('body').classList.contains('loading')){document.querySelector('body').classList.remove('loading');} });</script>
	    	<?php
	    	}
	    	$css = ob_get_clean();
	...
	function course_action_points(){
		...
		$action_controls = apply_filters('wplms_course_status_action_controls',array(
			'hide_timeline'=>array(
				'icon'=>'vicon vicon-angle-double-left',
				'title'=>_x('Hide Timeline','status','vibe'),
				),
			'fullscreen'=>array(
				'icon'=>'vicon vicon-fullscreen',
				'title'=>_x('Go fullscreen','status','vibe'),
				),
			));
		?>
			...
			<?php
				if(!empty($action_controls)){
					foreach($action_controls as $key => $control){
						?>
						<li class="<?php echo vibe_sanitizer($key,'text'); ?>">
							<?php 
								if(!empty($control['html'])){echo vibe_sanitizer($control['html']);}
								else{ ?>
								<a class="<?php echo vibe_sanitizer($control['icon'],'text'); ?> action_control"></a>
							<?php } ?>
						</li>
						<?php
					}
    	...
    	if($header_style == 'transparent' || $header_style == 'generic'){ 
	    	if(is_page() || is_single() || (function_exists('bp_is_directory') &&  bp_is_directory()) || (function_exists('bp_current_component') &&  bp_current_component()) || is_archive() || is_search() || (is_home() && !is_front_page())){ 
	    		...
	    		if(!is_archive() || bp_is_directory()){
	    			if(empty($post->ID) && isset($bp->pages->course->id)){
	    		...
	    		if(isset($title_bg) && is_numeric($title_bg)){
	    		...
	    		<style>.course_header,.group_header{
	    			<?php echo vibe_sanitizer($title_bg); ?>
	    			}#title{<?php echo vibe_sanitizer($title_bg); ?> padding-bottom:30px !important; background-size: cover;}

	    		#title.dark h1,#title.dark h5,#title.dark a:not(.button),#title.dark,#title.dark #item-admins h3,#item-header.dark #item-header-content .breadcrumbs li+li:before,#title.dark .breadcrumbs li+li:before,.group_header.dark div#item-header-content,.group_header.dark #item-header-content h3 a,.bbpress.dark .bbp-breadcrumb .bbp-breadcrumb-sep:after,#item-header.dark #item-admins h3,#item-header.dark #item-admins h5,#item-header.dark #item-admins h3 a,#item-header.dark #item-admins h5 a,
	    		#item-header.dark #item-header-content a,#item-header.dark #item-header-content{<?php echo vibe_sanitizer($title_link); ?>}
	    		#title.light h1,#title.light h5,#title.light a:not(.button),#title.light,#title.light #item-admins h3,#item-header.light #item-header-content .breadcrumbs li+li:before,#item-header.light #item-admins h3,#item-header.light #item-admins h5,#item-header.light #item-admins h3 a,#item-header.light #item-admins h5 a,#title.light .breadcrumbs li+li:before,.group_header.light div#item-header-content,.group_header.light #item-header-content h3 a,.bbpress.light .bbp-breadcrumb .bbp-breadcrumb-sep:after,#item-header.light #item-header-content a,#item-header.light #item-header-content{
	    			<?php echo vibe_sanitizer($title_color) .'!important;'; ?>
    			}
    ...
    function transparent_header_fix(){
    	...
    	if(($header_style == 'transparent' || $header_style == 'generic') && $course_status_template != 'fullscreen'){ 
    		?>
    ...
    function include_child_theme_styling(){
    	if (get_template_directory() !== get_stylesheet_directory()) {
	      	wp_enqueue_style('wplms_child_theme_style',get_stylesheet_uri(),'wplms-style',WPLMS_VERSION);
	    } 
    }
	...
	function course_search_results($query){
	  ...
	  if ( function_exists('get_coauthors')) {
	    if(isset($instructor) && $instructor !='*' && $instructor !='' && is_numeric($instructor)){
	      ...
	      //$query->query['author']=$instructor_name;
	      if(empty($query->query_vars['tax_query'])){
	      	$query->query_vars['tax_query'] = array('relation'=>'AND');
	      }
	      $query->query_vars['tax_query'][]= array(
	      	'taxonomy'=>'author',
	      	'field'=>'slug',
	      	'terms'=> 'cap-'.$instructor_name
	      );
	    }
	  }else{
	...
	*/
	...
	function wplms_check_single_activity_view(){
		if(function_exists('bp_is_single_activity') && !bp_is_single_activity())
			return;
		$flag=1;
		$single_activity_view=vibe_get_option('single_activity_view');
	  	if(isset($single_activity_view) && $single_activity_view){
		    $flag=0;
		    switch($single_activity_view){
		      case 1:
		        if(is_user_logged_in())$flag=1;
		      break;
		      case 2:
	...
	function get_course_unfinished_unit($course_id){
	    ...
	    if(function_exists('bp_course_check_unit_complete')){ 
	  	...
	  	$flag=apply_filters('wplms_next_unit_access',true,$units[$pre_unit_key],$user_id);
	        	...
	        	if(!empty($pre_unit_time)){
	            	...
	            	if($value > time()){
	                	...
	                	add_action('wplms_before_start_course_content',function()use($drip_unit_v){
	                    	...
	                    	echo '<div class="container top30"><div class="row"><div class="col-md-9"><div class="message"><p>'.sprintf(__('Next %s will be available in %s','vibe'),$this->element,$remaining).'</p></div>'.do_shortcode('[countdown_timer event="drip_feed_end" event_detail="'.$drip_unit_v.'" seconds="'.($remaining_secs).'" size="3"]').'</div></div></div>';
	                	});
	            	...
	            	if(isset($pre_unit_key )){
	                	...
	                	if(!empty($completed)){
	                    	...
	                    	add_action('wplms_before_start_course_content',function()use($drip_unit_v){
	                        	echo '<div class="container top30"><div class="row"><div class="col-md-9"><div class="message"><p>'.sprintf(__('Next %s will be available in %s','vibe'),$this->element,tofriendlytime($this->value)).'</p></div>'.do_shortcode('[countdown_timer event="drip_feed_end" event_detail="'.$drip_unit_v.'" seconds="'.($this->value).'" size="3"]').'</div></div></div>';
	                    	});
	...
	function wplms_signon(){
		...
		if(!isset($_POST['security']) || !wp_verify_nonce(trim($_POST['security']),'wplms_signon')){
	...
	function wplms_forgot_password(){
		...
		if ( function_exists('bp_send_email') && !bp_send_email( 'wplms_forgot_password',$user_email, $bpargs )){
	...
	function demo_import_fixes($customizer){
		...
		if($style == 'demo5'){
		    ...
		    header.standard nav>.menu>li:hover>a:after{
		      background:<?php echo vibe_sanitizer($primary_bg);?>
		    }
		    header #searchform:after, nav>.menu>li:hover>a:before{
		      border-color:transparent transparent <?php echo vibe_sanitizer($primary_bg);?> transparent !important;
		    }
		    header.standard{
		    border-bottom-color:<?php echo vibe_sanitizer($primary_bg);?> !important;
		    }
			<?php  
		}
