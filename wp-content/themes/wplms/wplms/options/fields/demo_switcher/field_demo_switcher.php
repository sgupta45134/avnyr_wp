...
class VIBE_Options_demo_switcher extends VIBE_Options{	
	...
	*/
	function render(){
		$ele_templates_not_available = array('default','demo12','demo_4','demo_4_academy','instructor','learningcenter','demo13','demo14','demo15','demo16','demo17','onecourse','oneinstructor','points_system','childone');
		...
		echo '<div id="demo_switcher_wrapper_margin"></div><div id="demo_switcher_wrapper"><fieldset>';
			...
			foreach($this->field['options'] as $k => $v){
				...
				'.((!empty($ele_templates_not_available) && !in_array($k,$ele_templates_not_available) && function_exists('did_action') && did_action( 'elementor/loaded' ))?'<a class="button button-primary import_ele_template import_demo import_demo_layout" data-demo="'.$k.'">'._x('Import Elementor Template','','vibe').'</a>':'').'
				</small>';
				echo '</label>';
